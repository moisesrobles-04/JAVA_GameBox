...
import Game.Zelda.Entities.Statics.MMRecursiveTile;
...
public class MapBuilder {
	...
	public static Map createMap(BufferedImage mapImage, Handler handler,String name){
		...
		for (int i = 0; i < mapImage.getWidth(); i++) {
			for (int j = 0; j < mapImage.getHeight(); j++) {
				...
				} else if(currentPixel==RecTileUp) {
					MMRecursiveTile Rec= new MMRecursiveTile(xPos,yPos,Images.zeldaTiles.get(30), handler);
					mapInCreation.addBlock(Rec);		
				} else if(currentPixel==RecTileRight) {
					MMRecursiveTile Rec= new MMRecursiveTile(xPos,yPos,Images.zeldaTiles.get(31), handler);
					mapInCreation.addBlock(Rec);		
				} else if(currentPixel==RecTileDown) {
					MMRecursiveTile Rec= new MMRecursiveTile(xPos,yPos,Images.zeldaTiles.get(32), handler);
					mapInCreation.addBlock(Rec);		
				} else if(currentPixel==RecTileLeft) {
					MMRecursiveTile Rec= new MMRecursiveTile(xPos,yPos,Images.zeldaTiles.get(33), handler);
					mapInCreation.addBlock(Rec);		
				}
			}
		}
		...
		for (int y = 0; y < info.get(0).size(); y++) {
			for (int x = 0; x < info.size(); x++) {
				if (Images.zeldaTiles.get(0).equals(info.get(x).get(y))){
					...
					//RecursiveTiles
				}else if(Images.zeldaTiles.get(30).equals(info.get(x).get(y))) {
					image.setRGB(x, y, RecTileUp);
				}else if(Images.zeldaTiles.get(31).equals(info.get(x).get(y))) {
					image.setRGB(x, y, RecTileRight);
				}else if(Images.zeldaTiles.get(32).equals(info.get(x).get(y))) {
					image.setRGB(x, y, RecTileDown);
				}else if(Images.zeldaTiles.get(33).equals(info.get(x).get(y))) {
					image.setRGB(x, y, RecTileLeft);
	...
	public static int RecTileUp = new Color(1,1,1).getRGB();
	public static int RecTileRight = new Color(1,1,2).getRGB();
	public static int RecTileDown = new Color(1,2,1).getRGB();
	public static int RecTileLeft = new Color(2,1,1).getRGB();
